...
@property(nonatomic, readwrite, NI_STRONG) NSMutableDictionary *operations;
@end
...
- (void)cancelOperation {
    [self.operations.allValues enumerateObjectsUsingBlock:^(NSOperation *operation, NSUInteger idx, BOOL *stop) {
        if ([operation isKindOfClass:[NIOperation class]]) {
            NIOperation *request = (NIOperation *) operation;
...
- (void)_didFinishLoadingWithImage:(UIImage *)image
    ...
    NSOperation *operation = [self.operations objectForKey:cacheIdentifier];
    if (operation == nil || operation.isCancelled) {
        [self.operations removeObjectForKey:cacheIdentifier];
        return;
    }
    ...
    [self.operations removeObjectForKey:cacheIdentifier];
    //NSLog(@"success, count:%d", self.operations.count);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)_didFailToLoadWithError:(NSError *)error cacheIdentifier:(NSString *)pathToNetworkImage {
    NSOperation *operation = [self.operations objectForKey:pathToNetworkImage];
    [self _didFailToLoadWithError:error opeartion:operation];
}

- (void)_didFailToLoadWithError:(NSError *)error opeartion:(NSOperation *)operation {
    NSArray *keys = [self.operations allKeysForObject:operation];
    [self.operations removeObjectsForKeys:keys];
    //NSLog(@"failes, count:%d", self.operations.count);
        ...
        if (nil != image) {
            ...
            AFImageRequestOperation *operation =
                    [AFImageRequestOperation imageRequestOperationWithRequest:request imageProcessingBlock:
                        ...
                        [self _didFailToLoadWithError:error cacheIdentifier:pathToNetworkImage];
                    }];
...
- (void)setNetworkImageOperation:(NIOperation <NINetworkImageOperation> *)operation forDisplaySize:(CGSize)displaySize contentMode:(UIViewContentMode)contentMode cropRect:(CGRect)cropRect {
    ...
    if (nil != operation) {
        ...
        if (nil != image) {
            ...
            [self.operations setObject:operation forKey:operation.cacheIdentifier];
